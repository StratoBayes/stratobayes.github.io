...
> This example also includes [Partitions](https://stratobayes.github.io/vignettes/Partitioning-data.html).
...
- $\gamma$ (sedimentation rates) -- As we don't have age estimates from $site2$, will use the log of this estimate as the mean of a normal distributed prior on $\ln(\gamma) = `{r} round(sedRateSite1, 2)`\,\text{m}/\text{Myr}$ on all partitions. To express considerable uncertainty, we set the standard deviation to $1\,\text{m}/\text{Myr}$. This implies a $95\%$ prior probability interval ranging from $0.14$ to $7$ times the mean. Higher or lower values may be sensible in other scenarios, depending on prior expectations of sedimentation rate variability.
...
## Next steps
- See [Partitioning data](https://stratobayes.github.io/vignettes/Partitioning-data.html) for a refresher on how  sedimentation rate changes and/or hiatuses enable more accurate correlations  
